        ...
        try {
            ...
            const uniqueResults = data.results.filter(
                (result: any, index: number, self: any[]) =>
                    result.success &&
                    self.findIndex((r: any) => r.ad === result.ad) === index
            );
            console.log("uniqueResults:", uniqueResults);

            if (uniqueResults.length === 0) {
                addToast("You have no claimable earnings at the moment. Clicks need to be processed before they can be withdrawn.", "info");
