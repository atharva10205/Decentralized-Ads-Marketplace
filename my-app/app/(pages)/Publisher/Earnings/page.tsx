...
//             const claimedAdIds = results.map((r: any) => r.ad);
...
const Earnings = () => {
    ...
    try {
        ...
        // ✅ Build claimSources — deduplicated by ad
        let claimSources: { ad: string; advertiser: string }[] = [];

        if (postResults.length > 0) {
            // Fresh attempt — POST gave us clean deduped results
            claimSources = postResults;
        } else {
            // Retry after cancel — deduplicate earningsRecords by ad
            // because GET returns one entry per click, same ad can appear multiple times
        ...
        if (simResult.err) {
            console.error("Simulation failed:", simResult.err, simResult.logs);
            // ✅ Log full simulation logs for easier debugging
